...
import {
  ...
  // Cost Insights APIs
  getCostInsightsSummary,
  getCostInsightsSuggestions,
  getCostInsightsModelComparison,
  getCostInsightsForecast,
  // Transform functions
...
// ============================================================================
// Cost Insights API Functions
// ============================================================================

describe('API - Cost Insights', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  describe('Cost Summary', () => {
    const mockSummary = { current_month_cost: 45.0, projected_monthly_cost: 50.0 };

    it('should get cost insights summary', async () => {
      mockApiClient.get.mockResolvedValue(mockSummary);

      const result = await getCostInsightsSummary();

      expect(mockApiClient.get).toHaveBeenCalledWith('/api/v1/cost-insights/summary');
      expect(result.current_month_cost).toBe(45.0);
    });
  });

  describe('Cost Suggestions', () => {
    const mockSuggestions = [{ id: 'sug-1', type: 'model_switch', estimated_savings: 10.0 }];

    it('should get cost suggestions', async () => {
      mockApiClient.get.mockResolvedValue(mockSuggestions);

      const result = await getCostInsightsSuggestions();

      expect(mockApiClient.get).toHaveBeenCalledWith('/api/v1/cost-insights/suggestions');
      expect(result[0].estimated_savings).toBe(10.0);
    });
  });
  ...
  describe('Model Comparison', () => {
    ...
    it('should get model comparison', async () => {
      mockApiClient.get.mockResolvedValue(mockComparison);

      const result = await getCostInsightsModelComparison();

      expect(mockApiClient.get).toHaveBeenCalledWith('/api/v1/cost-insights/model-comparison');
  ...
  describe('Cost Forecast', () => {
    it('should get cost forecast', async () => {
      const mockForecast = { projected_cost: 60.0, confidence: 0.85 };
      mockApiClient.get.mockResolvedValue(mockForecast);

      const result = await getCostInsightsForecast();

      expect(mockApiClient.get).toHaveBeenCalledWith('/api/v1/cost-insights/forecast');
