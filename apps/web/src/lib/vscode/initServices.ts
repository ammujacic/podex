...
function applyTerminalNoirTheme() {
  ...
  try {
    ...
    style.textContent = `
      ...
      /* Default text color - white for all plain/default text */
      .monaco-editor {
...
async function loadServiceOverrides() {
  const [
    ...
    import('@codingame/monaco-vscode-extensions-service-override').then((m) => m.default),
  ]);
 ...
 *
 * Note: Errors during extension loading are caught and logged but don't
 * prevent the editor from working. This handles race conditions in the
 * language service where callbacks may fire after disposal.
 */
async function loadGrammarExtensions() {
  try {
    ...
    console.warn('[VSCode] Theme extension error (non-fatal):', error);
  }
  ...
  // Wrapped in try-catch to handle race conditions in language configuration
  try {
    await Promise.all([
      // Web languages
    ...
    // Non-fatal: language extensions may fail due to race conditions in
    // _loadConfigurationsForMode when the service is disposed during init.
    // The editor will still work, just without some language features.
    console.warn('[VSCode] Language extension error (non-fatal):', error);
  }
}
...
export async function initializeVSCodeServices(): Promise<void> {
  ...
  initializationPromise = (async () => {
    try {
      ...
      // The extensions service override is required for loading VSCode extensions
      // (including theme-defaults) which use the extension-file:// URI scheme
