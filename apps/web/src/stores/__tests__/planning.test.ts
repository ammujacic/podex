...
describe('planningStore', () => {
  ...
  describe('Initial State', () => {
    ...
    it('has default settings configured', () => {
      const { result } = renderHook(() => usePlanningStore());
      expect(result.current.settings.planningModel).toBe('claude-opus-4-20250514');
      expect(result.current.settings.executionModel).toBe('claude-sonnet-4-20250514');
  ...
  describe('Settings', () => {
    describe('updateSettings', () => {
      ...
      it('updates execution model', () => {
        ...
        act(() => {
          result.current.updateSettings({ executionModel: 'claude-opus-4-20250514' });
        });

        expect(result.current.settings.executionModel).toBe('claude-opus-4-20250514');
      });
