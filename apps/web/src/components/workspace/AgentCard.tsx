...
export function AgentCard({ agent, sessionId, expanded = false }: AgentCardProps) {
  ...
  // Audio playback
  const { isPlaying, playingMessageId, playAudioUrl, playAudioBase64, stopPlayback, unlockAudio } =
    useAudioPlayback({
      sessionId,
      onPlayEnd: () => {},
    });
  ...
  const handlePlayMessage = useCallback(
    async (messageId: string) => {
      ...
      try {
        const result = await synthesizeMessage(sessionId, agent.id, messageId);
        if (result.audio_b64) {
  ...
  // Voice cancel - clear pending flag and cancel recording
  const handleVoiceCancel = useCallback(() => {
    pendingVoiceSendRef.current = false;
    cancelRecording();
  }, [cancelRecording]);

  // Voice recording - mark for auto-send when final transcript arrives
  ...
  useEffect(() => {
    if (!pendingVoiceSubmit || isSending) return;
    ...
    const submitVoiceMessage = async () => {
      ...
      if (
        lastSentMessageRef.current &&
        lastSentMessageRef.current.content === transcript &&
        now - lastSentMessageRef.current.timestamp < 2000
      ) {
        return;
      }
      lastSentMessageRef.current = { content: transcript, timestamp: now };

      // Use handleSendMessage with the transcript
      handleSendMessage(transcript);
    };

    submitVoiceMessage();
  }, [pendingVoiceSubmit, isSending, handleSendMessage]);
  ...
  return (
    ...
    >
      ...
      {/* Input area */}
      <div className="relative border-t border-border-subtle p-3" data-tour="agent-input">
        ...
        {/* Attachment previews */}
        {attachments.length > 0 && !isRecording && !isTranscribing && (
        ...
        >
          ...
          {/* Mic button - only show when TTS is enabled */}
          {ttsEnabled && (
            <button
              ...
              className={cn(
                'rounded-md p-2 transition-colors touch-none select-none',
                isRecording
          ...
          <div className="relative flex-1">
            <input
              ...
              placeholder={`Type / for commands or ask ${agent.name.toLowerCase()}...`}
