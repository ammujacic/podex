    ...
    while True:
        try:
            ...
            try:
                ...
                # Cleanup orphaned workspace directories (directories without Redis entries)
                try:
                    orphaned = await manager.cleanup_orphaned_directories()
                    if orphaned:
                        logger.info("Cleaned up orphaned directories", count=len(orphaned))
                except Exception:
                    logger.exception("Error cleaning up orphaned directories")
            finally:
