...
class MultiServerDockerManager:
    ...
    async def list_workspace_directories(self, server_id: str) -> list[str]:
        """List all workspace directory IDs on a server.

        Args:
            server_id: Server identifier

        Returns:
            List of workspace IDs found in the workspace data path
        """
        conn = self._connections.get(server_id)
        if not conn:
            return []
        ...
        if settings.environment == "development":
            # In dev, list via a temporary container on the DinD server
            client = self.get_client(server_id)
            if not client:
                return []
            ...
            return await loop.run_in_executor(None, _list_dirs)

        # Production: SSH to list
        loop = asyncio.get_event_loop()

        def _ssh_list() -> list[str]:
            try:
                ls_cmd = [
                    "ssh",
                    "-o",
                ...
                result = subprocess.run(  # noqa: S603
                    ls_cmd, capture_output=True, timeout=30, check=False
                )
                output = result.stdout.decode().strip() if result.stdout else ""
                # Filter to only UUID-like directory names (36 chars)
                return [d for d in output.split("\n") if d and len(d) == 36]
            except Exception:
                return []

        return await loop.run_in_executor(None, _ssh_list)
