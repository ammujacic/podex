...
class ComputeManager(ABC):
    ...
    async def cleanup_deleted_workspaces(self) -> list[str]:
        ...
        for i, workspace in enumerate(workspaces_to_cleanup, 1):
            ...
            try:
                # preserve_files=False to also remove workspace directories
                await self.delete_workspace(workspace.id, preserve_files=False)
    ...
    async def cleanup_orphaned_directories(self) -> list[str]:
        ...
        Note: This is a no-op in the base class. Override in subclasses that have
        direct access to the file system (e.g., MultiServerComputeManager).
        """
        return []
