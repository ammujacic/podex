...
def _build_agent_response(
    ...
    *,
    skip_conversation_load: bool = False,
) -> AgentResponse:
    ...
    Args:
        ...
        skip_conversation_load: If True, skip accessing attached_conversation to avoid
            lazy loading in async context. Use for newly created agents that have no
            attached conversation yet.

    Returns:
        AgentResponse with all fields populated
    """
    # Get conversation_session_id from the attached_conversation relationship
    # Skip if explicitly told to (e.g., for newly created agents)
    conversation_session_id = None
    if not skip_conversation_load and agent.attached_conversation:
...
async def create_agent(
    ...
    # Re-fetch agent with relationship eagerly loaded to avoid lazy loading in response
    result = await db.execute(
        select(AgentModel)
    ...
    # skip_conversation_load=True since newly created agents never have an attached conversation
    return _build_agent_response(agent, model_display_name, skip_conversation_load=True)
...
async def duplicate_agent(
    ...
    # skip_conversation_load=True since duplicated agents start without an attached conversation
    return _build_agent_response(new_agent, model_display_name, skip_conversation_load=True)
