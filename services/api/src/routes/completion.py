...
### Completion:"""
    ...
    try:
        ...
        # Default to OpenRouter's Claude Haiku for fast inline completions
        model_id = body.model or "openrouter:anthropic/claude-3-5-haiku"
...
CONCEPTS: [Comma-separated list of key concepts]"""
    ...
    try:
        # Use multi-provider completion with usage tracking
        # Default to OpenRouter's Claude Haiku for explanations
        model_id = body.model or "openrouter:anthropic/claude-3-5-haiku"
        result = await CompletionProvider.complete_with_usage(
            prompt=prompt,
...
Return ONLY valid JSON, no other text. If no issues found, return: []"""
    ...
    try:
        ...
        # Default to OpenRouter's Claude Haiku for bug detection
        model_id = body.model or "openrouter:anthropic/claude-3-5-haiku"
        result = await CompletionProvider.complete_with_usage(
            prompt=prompt,
