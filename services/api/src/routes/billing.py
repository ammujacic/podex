...
async def get_or_create_stripe_customer(db: AsyncSession, user: User) -> str:
    ...
    # Note: Don't flush here - let the request's auto-commit handle it.
    # Flushing clears session.dirty, causing get_db() to skip commit.
...
class UsageEventInput(BaseModel):
    ...
    # Cost fields are optional - API calculates pricing server-side
    unit_price_cents: int = Field(default=0, ge=0, le=settings.MAX_COST_CENTS)
    total_cost_cents: int = Field(default=0, ge=0, le=settings.MAX_COST_CENTS)
...
def _calculate_token_cost_from_db(
    ...
    Uses database pricing only - no fallbacks. Models must be registered.
    ...
    Returns:
        ...
        Returns 0 if model pricing not found (usage still tracked, but check logs).
    """
    import math
    ...
    if not pricing:
        # No fallbacks - model must be in database
        # Log error so we know to add the model, but don't fail the request
        # Usage is still tracked with $0 cost for auditing purposes
        if model:
            logger.error(
                "Model pricing not found in database - usage tracked with $0 cost",
                model=model,
