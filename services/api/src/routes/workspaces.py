...
@router.post("/{workspace_id}/stop", response_model=WorkspaceStatusResponse)
@limiter.limit(RATE_LIMIT_STANDARD)
async def stop_workspace(
    ...
    Stops the workspace container gracefully. The workspace data is preserved
    and can be restarted later with the /start endpoint.
    """
    from src.exceptions import ComputeServiceHTTPError
    from src.services.workspace_router import workspace_router
    ...
    # Only allow stopping running workspaces
    if workspace.status not in ("running", "pending"):
        raise HTTPException(
            status_code=400,
            detail=f"Cannot stop workspace in '{workspace.status}' state. "
            "Only running or pending workspaces can be stopped.",
        )
    ...
    try:
        ...
        logger.info("Workspace stopped", workspace_id=workspace_id, user_id=user_id)

    except ComputeServiceHTTPError as e:
        await db.rollback()
        logger.exception(
            "Compute service error during stop",
            workspace_id=workspace_id,
            status_code=e.status_code,
        )
        raise HTTPException(
            status_code=500,
            detail=f"Failed to stop workspace: compute service error ({e.status_code})",
        ) from None
    except HTTPException:
        await db.rollback()
        raise
        ...
        await db.rollback()
        logger.exception("Failed to stop workspace", workspace_id=workspace_id)
        raise HTTPException(status_code=500, detail="Failed to stop workspace") from None
