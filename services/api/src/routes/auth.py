...
def _parse_device_info(request: Request) -> dict[str, str | None]:
    ...
    # Get IP address (handle proxies and CDNs)
    # Check headers in order: CF-Connecting-IP, X-Real-IP, X-Forwarded-For, then client.host
    ip_address: str | None = request.headers.get("cf-connecting-ip")
    if not ip_address:
...
async def _create_device_session(
    ...
    city, country, country_code = await lookup_ip_location(device_info["ip_address"])
    ...
    for sub in active_subscriptions:
        if sub.stripe_subscription_id:
            try:
                ...
                stripe.Subscription.delete(sub.stripe_subscription_id)  # type: ignore[arg-type]
                logger.info(
