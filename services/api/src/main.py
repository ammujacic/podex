...
async def _run_with_startup_lock(
    task_name: str,
    run_task: Callable[[], Awaitable[None] | None],
    lock_ttl: int = 300,
    complete_ttl: int = 60,
) -> None:
    """Run a startup task with distributed locking to prevent concurrent execution.

    Uses Redis to ensure only one worker runs the task. Other workers wait
    for the task to complete before proceeding.

    Args:
        task_name: Unique name for the task (used in Redis keys)
        run_task: The task function to run (sync or async)
        lock_ttl: Max time in seconds for task to complete before lock expires
        complete_ttl: How long to keep the "complete" flag (for other workers to see)
    """
    from src.middleware.rate_limit import get_redis_client

    lock_key = f"podex:{task_name}:lock"
    complete_key = f"podex:{task_name}:complete"

    try:
        redis = await get_redis_client()

        # Check if task already completed by another worker in this deployment
        if await redis.get(complete_key):
            logger.info(
                "Startup task already completed by another worker, skipping", task=task_name
            )
            return None
        ...
        if acquired:
            ...
            logger.info("Acquired startup task lock, running", task=task_name)
            try:
                result = run_task()
                if asyncio.iscoroutine(result):
                    await result
                # Mark task as complete so other workers skip
                await redis.set(complete_key, "1", ex=complete_ttl)
                logger.info("Startup task complete, notified other workers", task=task_name)
            finally:
                # Release the lock
                await redis.delete(lock_key)
        else:
            # Another worker is running the task - wait for completion
            logger.info("Another worker is running startup task, waiting...", task=task_name)
            ...
            while waited < max_wait:
                ...
                if await redis.get(complete_key):
                    logger.info("Startup task completed by another worker", task=task_name)
                    return None

                # Check if lock was released (worker crashed without completing)
                if not await redis.get(lock_key):
                    # Lock released but not complete - try to acquire and run
                    logger.warning(
                        "Startup task lock released without completion, retrying", task=task_name
                    )
                    return await _run_with_startup_lock(task_name, run_task, lock_ttl, complete_ttl)

                await asyncio.sleep(poll_interval)
                waited += poll_interval

            # Timeout - proceed anyway (task might be stuck)
            logger.error("Timed out waiting for startup task, proceeding", task=task_name)

    except Exception as e:
        # Redis unavailable - fall back to running task (might duplicate but should be idempotent)
        logger.warning(
            "Redis unavailable for startup task lock, running anyway", task=task_name, error=str(e)
        )
        result = run_task()
        if asyncio.iscoroutine(result):
            await result
...
async def run_migrations_with_lock() -> None:
    """Run migrations with distributed locking to prevent concurrent execution."""
    await _run_with_startup_lock("migrations", run_migrations)


async def seed_database_with_lock() -> None:
    """Run database seeding with distributed locking to prevent concurrent execution."""
    await _run_with_startup_lock("seeding", seed_database)
    ...
    while True:
        try:
            ...
            try:
                ...
                # NOTE: Don't release lock early - let TTL handle it.
                # Prevents staggered workers from running redundantly.
                pass
    ...
    while True:
        try:
            ...
            try:
                ...
                # NOTE: Don't release lock early - let TTL handle it (120s).
                # This prevents staggered workers from each acquiring the lock
                # within the same cycle when the task completes quickly.
                pass
...
async def lifespan(_app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    # Uses distributed lock to ensure only one worker runs migrations
    ...
    # Uses distributed lock to ensure only one worker runs seeding
    await seed_database_with_lock()
