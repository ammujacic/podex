...
class UsageEvent(BaseModel):
    ...
    # Usage source: "included" (OpenRouter/platform), "external" (user API key),
...
class TokenUsageParams:
    ...
    Note: Pricing is NOT required here. The API calculates pricing server-side
    using the model registry to ensure accurate billing and prevent manipulation.
    This matches the compute usage pattern.
    """
...
class UsageTracker:
    ...
    async def record_token_usage(self, params: TokenUsageParams) -> UsageEvent:
        ...
        Note:
            ...
            The API service calculates pricing server-side based on the model
