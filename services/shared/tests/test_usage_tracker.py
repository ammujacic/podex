...
class TestUsageTrackerRecordTokenUsage:
    ...
    async def test_record_token_usage(self) -> None:
        ...
        # Pricing is calculated server-side, not client-side

    @pytest.mark.asyncio
    async def test_record_token_usage_unknown_model_accepted(self) -> None:
        """Test recording token usage with unknown model is accepted.

        Since pricing is calculated server-side, unknown models are accepted
        by the client. The server will validate and calculate pricing.
        """
        tracker = UsageTracker(api_base_url="http://localhost:8000", batch_size=100)
        params = TokenUsageParams(
            user_id="user-123",
        ...
        # Unknown models are accepted - pricing is calculated server-side
        event = await tracker.record_token_usage(params)
...
class TestUsageTrackerRecordStorageUsage:
    ...
    async def test_record_storage_usage(self) -> None:
        ...
        # Pricing is calculated server-side by the API
