...
setup_dotfiles() {
    ...
    for dotfile in "${dotfiles[@]}"; do
        ...
        if [[ -e "${dest}" && ! -L "${dest}" && -e "${src}" ]]; then
            ...
            # Use -container-backup suffix (not .container-backup) to avoid creating
            # a path inside the directory when dest is a directory
            local backup_path="${dest%/}-container-backup"
            mv "${dest}" "${backup_path}" 2>/dev/null || rm -rf "${dest}"
            ln -s "${src}" "${dest}"
